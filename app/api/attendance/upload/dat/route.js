...
import {
  calculateNumOfHours,
  checkIfDataExistsMysqlDB,
  compareTimes,
  subtractTimes,
} from "@/utils";
import { namesWithId } from "@/data/data";
import { createAndInsertAttendanceTableMysql } from "@/lib/mysqldb";
...
export async function POST(req) {
  ...
  for (const entry of filteredLines) {
    ...
    const doesDataExist = await checkIfDataExistsMysqlDB(employee_id, date);
  ...
  createAndInsertAttendanceTableMysql(result);
